...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    CALC_VERIFIED_USER(state, action) {
      ...
      const usersArray = state && state.users ? state.users : [];

      const verifiedUsersCount = usersArray.reduce((count, user) => {
        return user.isVerified ? count + 1 : count;
      }, 0);

      state.verifiedUsers = verifiedUsersCount;
    },
    ...
    CALC_SUSPENDED_USER(state, action) {
      ...
      const suspendedUsersCount = usersArray.reduce((count, user) => {
        return user.role === targetRole ? count + 1 : count;
      }, 0);

      state.suspendedUsers = suspendedUsersCount;
    },
  },
